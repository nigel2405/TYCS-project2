...
const AdminDashboard = () => {
  ...
  const [selectedSessionChat, setSelectedSessionChat] = useState(null);
  ...
  const fetchGPUs = async () => {
    try {
      const res = await axios.get('/admin/gpus');
  ...
  const handleTerminateSession = async (sessionId) => {
    if (!window.confirm('WARNING: Forcefully terminate this active session? This will immediately disconnect the consumer.')) return;
    try {
      await axios.post(`/admin/sessions/${sessionId}/terminate`);
      fetchSessions();
      fetchStats();
      fetchGPUs(); // GPU might be freed
      alert('Session forcefully terminated.');
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to terminate session');
    }
  };

  const handleUnlistGPU = async (gpuId) => {
    if (!window.confirm('Forcefully unlist this GPU? It will no longer appear in the marketplace.')) return;
    try {
      await axios.put(`/admin/gpus/${gpuId}/unlist`);
      fetchGPUs();
      fetchStats();
      alert('GPU unlisted successfully.');
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to unlist GPU');
    }
  };
  ...
  const handleBanUser = async (userId, isActive) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || `Failed to ${action.toLowerCase()} user`);
    }
  };

  const handleViewChat = async (sessionId) => {
    try {
      const res = await axios.get(`/admin/sessions/${sessionId}/messages`);
      setChatMessages(res.data.data.messages || []);
      setSelectedSessionChat(sessionId);
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to fetch chat history');
    }
  };
    ...
    <div ref={pageRef} className="relative z-10" style={{ opacity: 0 }}>
      ...
      <div className="max-w-7xl mx-auto px-5 pb-12">
        ...
        <div className="flex gap-3 mb-8 border-b-2 border-white/10">
          {['stats', 'users', 'gpus', 'sessions', 'payouts'].map((tab) => (
            <button
              key={tab}
              className={`px-6 py-3 text-base font-medium transition-colors border-b-2 ${activeTab === tab
                ? 'text-primary-500 border-primary-500'
                : 'text-white/60 border-transparent hover:text-primary-500 hover:border-primary-500/50'
                }`}
              onClick={() => setActiveTab(tab)}
            >
              {tab === 'stats' ? 'Platform Stats' : tab === 'users' ? 'Users' : tab === 'gpus' ? 'GPUs' : tab === 'sessions' ? 'Sessions' : 'Payouts'}
            </button>
          ))}
        </div>
                    ...
                    <tr key={user.id || user._id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3 flex gap-2">
                        ...
                        <GlassButton
                          ...
                          size="sm"
                          onClick={() => handleBanUser(user.id || user._id, user.isActive)}
                        >
        ...
        {activeTab === 'gpus' && (
          <GlassCard className="cursor-glow" cursorGlow>
            <h2 className="text-2xl font-bold text-white mb-6">All GPUs</h2>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead className="glass-card bg-white/5">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-white border-b border-white/10">Name</th>
                    <th className="px-4 py-3 text-left font-semibold text-white border-b border-white/10">Provider</th>
                    <th className="px-4 py-3 text-left font-semibold text-white border-b border-white/10">VRAM</th>
                ...
                <tbody>
                  {gpus.map((gpu) => (
                    <tr key={gpu._id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      <td className="px-4 py-3 text-white/90">{gpu.name} ({gpu.model})</td>
                      <td className="px-4 py-3 text-white/70">{gpu.provider?.username || 'Unknown'}</td>
                      <td className="px-4 py-3 text-white/70">{gpu.vram} GB</td>
                      <td className="px-4 py-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${gpu.isActive ? 'bg-success-500' : 'bg-red-600'} text-white`}>
                          {gpu.isActive ? (gpu.isAvailable ? 'Available' : 'In Use') : 'Unlisted'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-secondary-500 font-semibold">${gpu.pricePerHour?.toFixed(2)}/hr</td>
                      <td className="px-4 py-3">
                        {gpu.isActive && (
                          <GlassButton variant="danger" size="sm" onClick={() => handleUnlistGPU(gpu._id)}>
                            Unlist
                          </GlassButton>
                        )}
                      </td>
                    ...
                    <tr key={session._id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3 flex gap-2">
                        {session.status === 'active' && (
                          <GlassButton variant="danger" size="sm" onClick={() => handleTerminateSession(session._id)}>
                            Kill Switch
                          </GlassButton>
                        )}
                        <GlassButton variant="primary" size="sm" onClick={() => handleViewChat(session._id)}>
                          View Chat
                        </GlassButton>
                      </td>
                    </tr>
      ...
      {selectedSessionChat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-gray-900 border border-white/20 rounded-2xl p-6 shadow-2xl relative">
            <button
              onClick={() => setSelectedSessionChat(null)}
            ...
            <h2 className="text-2xl font-bold text-white mb-6">Session Chat History</h2>

            <div className="h-96 overflow-y-auto pr-2 space-y-4 mb-6 custom-scrollbar">
              {chatMessages.length === 0 ? (
                <div className="text-white/50 text-center py-10">No messages found for this session.</div>
              ) : (
                chatMessages.map((msg) => (
                  <div key={msg._id} className="bg-white/5 p-3 rounded-xl border border-white/10">
                    <div className="flex justify-between items-center mb-1">
                      <span className={`text-sm font-semibold ${msg.sender?.role === 'provider' ? 'text-primary-400' : 'text-success-400'}`}>
